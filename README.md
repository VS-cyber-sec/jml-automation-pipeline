...
```bash
git clone https://github.com/YOUR-USERNAME/jml-automation-pipeline.git
...
**Root cause:** Using a Regular Web App Client ID instead of a
Machine-to-Machine app. Web apps cannot use `client_credentials` grant.

**Fix:** Create a dedicated M2M app authorized for the Management API
with correct permissions (`read:users`, `update:users`, `read:roles`,
`create:role_members`).

**Learning:** The OAuth `client_credentials` grant is for machine-to-machine
authentication only. Regular web apps use Authorization Code flow where
a human is present. Using the wrong app type causes a 401 even with
correct credentials.
...
**Root cause:** Used `api.http.post()` — Auth0 Actions do not have a
built-in `api.http` object. The correct approach is to add `axios`
as an explicit module dependency.

**Fix:** Add `axios 0.27.2` in the Modules panel → use
`const axios = require("axios")` at the top of the Action.

**Learning:** Auth0 Actions run in an isolated Node.js environment.
Only explicitly declared modules are available. Always check the
Modules panel before using any external library.
...
**Root cause:** Test payload used synthetic user ID `auth0|test123`
which does not exist in the Auth0 directory. The Management API cannot
assign roles to a non-existent user.

**Fix:** Use a real user ID from Auth0 Dashboard → Users → copy the
`user_id` field. In production this never occurs because the Action
fires after the real user is created.

**Learning:** The Post User Registration Action fires in the context of
a real registration event. The test panel simulates the trigger but uses
synthetic data — real user IDs must be used for API calls that require
directory lookup.
...
**Root cause:** `.env` file had comments (`# comment`) or blank lines
that `python-dotenv` could not parse — causing all values to load as `None`.

**Fix:** Remove all comments and blank lines from `.env`. No spaces
around `=`. No `#` characters anywhere in the file.

**Learning:** Secrets in Auth0 Actions are stored encrypted in Auth0's
vault and injected at runtime via `event.secrets`. They are NOT read
from `.env` — that file is only for local development reference.
...
| Timestamp | Event | User Email | Department | Roles Assigned | Status |
|-----------|-------|------------|------------|----------------|--------|
| 2026-05-27T10:30:00Z | New Hire Onboarded | jane.engineer@example.com | Engineering | all-employees, engineering | Success |
| 2026-05-27T11:15:00Z | New Hire Onboarded | rahul.hr@example.com | HR | all-employees, hr-finance | Success |
| 2026-05-27T14:22:00Z | New Hire Onboarded | priya.sales@example.com | unknown | all-employees | Success |

### Compliance mapping

| Framework | Requirement | How this project satisfies it |
|-----------|-------------|-------------------------------|
| **SOC 2 CC6** | Logical access controls | Role-based access assigned automatically with full audit log |
| **ISO 27001 A.9** | Access control | Least Privilege enforced — only role-specific access granted |
| **GDPR** | Data minimisation | Only necessary attributes read; no PII stored beyond audit log |
| **Zero Trust** | Never trust, always verify | Access granted based on verified identity attributes, not assumption |

### Auth0 System Log

Every Action execution is logged in Auth0:

**Reports → Logs → filter: `Post User Registration`**
...
```json
{
  ...
  "description": "Post User Registration Action executed",
  "user_name": "jane.engineer@example.com",
  "details": {
    "actions": ["JML New Hire Onboarding"]
  }
}
```
...
This project was built specifically to demonstrate competencies
required for the **IAM Engineer — Okta & Microsoft Entra** role at Cohesity.

| JD Requirement | Implementation | Evidence |
|----------------|---------------|----------|
| Okta Workflows (mandatory) | Auth0 Actions (Okta platform) | `src/actions/jml-new-hire-action.js` |
| JML lifecycle ownership | Complete Joiner flow automated | Flow diagram + test results |
| Provisioning automation | Role assignment via Management API | Roles tab in Auth0 |
| De-provisioning concepts | Leaver architecture documented | `docs/JML-CONCEPTS.md` |
| Audit logging | Google Sheets + Auth0 System Log | `audit-samples/` folder |
| Exception handling | try/catch on every API call | Error logs in debugging section |
| Least Privilege | Engineering users never get HR roles | Test verified |
| AI tool usage | Claude AI used throughout learning | Acknowledged in docs |
| Zero Trust principles | Access based on verified attributes | Architecture diagram |
| SCIM concepts | Documented in architecture | `docs/ARCHITECTURE.md` |
...
1. **OAuth grant types matter** — `client_credentials` is for M2M only.
   Using a web app Client ID for server-side automation causes 401.

2. **Auth0 Action modules must be explicit** — no built-in HTTP client.
   axios must be declared in the Modules panel before requiring it.

3. **Test payloads need real data** — synthetic user IDs cause 404 on
   Management API calls that require directory lookup.

4. **Secrets in Actions are injected at runtime** — not from `.env`.
   The Secrets panel encrypts and injects them as `event.secrets`.

5. **Post User Registration is post-creation** — the user exists in
   Auth0 directory when the Action fires. Role assignment is safe.

### Conceptual

1. **JML is the backbone of enterprise IAM** — every security
   framework (SOC 2, ISO 27001, Zero Trust) requires it.

2. **Least Privilege is not a one-time config** — it must be enforced
   at every lifecycle event, not just initial provisioning.

3. **Audit trails are non-negotiable** — not for operations, but for
   proving to auditors that access was granted correctly and promptly.

4. **Auth0 and Okta are the same platform** — concepts transfer
   directly. Architecture, triggers, conditions, actions — identical.

---

## 🔮 What's Next (Roadmap)

- [ ] **Mover flow** — trigger on department attribute change,
      remove old roles, assign new roles
- [ ] **Leaver flow** — trigger on user deactivation,
      revoke all roles, log offboarding timestamp
- [ ] **Approval gate** — pause flow, send manager approval request,
      resume on approval (simulating Okta Workflows approval step)
- [ ] **SCIM simulation** — script to deprovision downstream app
      accounts on user deactivation
- [ ] **OIG simulation** — periodic access review script using
      Management API to flag users with excess roles
- [ ] **Okta migration** — replicate this pipeline in Okta Workflows
      when Okta tenant access is available
...
**Vaishnavi Chavan**
Final-year Computer Engineering student | Minor in Cybersecurity
Aspiring IAM Engineer — Okta & Microsoft Entra
...
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=flat-square&logo=github)](https://github.com/VS-cyber-sec)

---

<div align="center">

**Built as part of hands-on IAM Engineer interview preparation for Cohesity**

*Auth0 is owned by Okta. All concepts demonstrated here are directly transferable to Okta Workflows.*
